...
public class TurnController implements CRUDController<TurnDTO> {
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "201", description = "Turn created", content = { @Content(mediaType = "application/json", schema = @Schema(implementation = TurnDTO.class)) }),
    ...
    @PostMapping("/new")
    public ResponseEntity<HttpStatus> register(@RequestBody TurnDTO turn) throws ResourceNotFoundException, IntegrityDataException, NoSuchDataExistsException, DataAlreadyExistsException {
        turnService.create(turn);
        return new ResponseEntity<>(HttpStatus.CREATED);
    }

    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "List of turns", content = { @Content(mediaType = "application/json", array = @ArraySchema(schema = @Schema(implementation = TurnDTO.class)) )}),
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Turn found", content = { @Content(mediaType = "application/json", schema = @Schema(implementation = TurnDTO.class)) }),
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Turn found", content = { @Content(mediaType = "application/json", array = @ArraySchema(schema = @Schema(implementation = TurnDTO.class)) )}),
            @ApiResponse(responseCode = "404", description = "Turn wasn't found", content = @Content),
            @ApiResponse(responseCode = "400", description = "The input format is incorrect", content = @Content )
    })
    @Operation(summary = "Get turn by dentist's registration number")
    @GetMapping("/dentistRegistrationNumber/{registrationNumber}")
    public ResponseEntity<List<TurnDTO>> searchByDentist(@PathVariable Long registrationNumber) throws IntegrityDataException, NoSuchDataExistsException {
    ...
    public ResponseEntity<List<TurnDTO>> searchByPatientDni(@PathVariable Long dni) throws IntegrityDataException, NoSuchDataExistsException {
    ...
    @PutMapping("/update")
    public ResponseEntity<TurnDTO> modify(@RequestBody TurnDTO turn) throws ResourceNotFoundException, IntegrityDataException, NoSuchDataExistsException, DataAlreadyExistsException {
