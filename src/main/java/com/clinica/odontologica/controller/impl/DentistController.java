...
public class DentistController implements CRUDController<DentistDTO> {
        ...
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "201", description = "Dentist created", content = {
        ...
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "200", description = "List of dentists", content = {
                                        @Content(mediaType = "application/json", array = @ArraySchema(schema = @Schema(implementation = DentistDTO.class))) }),
        ...
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "200", description = "Dentist found", content = {
                                        @Content(mediaType = "application/json", schema = @Schema(implementation = DentistDTO.class)) }),
        ...
        public ResponseEntity<DentistDTO> searchByRegistrationNumber(@PathVariable Long registrationNumber)
                        throws ResourceNotFoundException, IntegrityDataException, NoSuchDataExistsException {
        ...
        public ResponseEntity<DentistDTO> searchByFullname(@PathVariable String firstname,
        ...
        @PutMapping("/update")
        public ResponseEntity<DentistDTO> modify(@RequestBody DentistDTO dentist)
                        throws ResourceNotFoundException, IntegrityDataException, NoSuchDataExistsException {
                return ResponseEntity.ok(dentistService.update(dentist));
        }

        @ApiResponses(value = {
                        @ApiResponse(responseCode = "200", description = "Dentist deleted", content = {
                                        @Content(mediaType = "application/json", schema = @Schema(implementation = DentistDTO.class)) }),
                        @ApiResponse(responseCode = "404", description = "Dentist wasn't found", content = @Content),
                        @ApiResponse(responseCode = "400", description = "The input format is incorrect", content = @Content)
        })
        ...
        public ResponseEntity<String> removeNroMatricula(@PathVariable Long registrationNumber)
                        throws ResourceNotFoundException, IntegrityDataException, NoSuchDataExistsException {
                dentistService.deleteByRegistrationNumber(registrationNumber);
                return ResponseEntity.status(HttpStatus.OK)
                                .body("Dentist with registration number: " + registrationNumber + " deleted");
        }
}
